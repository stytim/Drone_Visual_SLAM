...
ControlNode::ControlNode()
{
	...
	//---Jim---
	freq_sub = nh_.subscribe("/ardrone/navdata_freq",50, &ControlNode::freqCb, this);
	freq_index = 20;
	freq_com = 3;
...
//---Jim---
void ControlNode::freqCb(const std_msgs::String::ConstPtr& msg)
{
	freq_index =  atoi( msg->data.c_str() );
}
...
void ControlNode::sendControlToDrone(ControlCommand cmd)
{
	...
	if(isControlling)
	{
		...
		if(freq_index <= 17)
		{	
			cmdT.angular.z = cmdT.linear.z = cmdT.linear.y = cmdT.linear.x = 0;
			freq_com = 0;
		}
		if(freq_index > 17 && freq_com < 3)
			freq_com++;
		if(freq_index > 17 && freq_com >= 3)
		{
