...
EstimationNode::EstimationNode()
{
    ...
    //---Jim---
    pose_channel = nh_.resolveName("tum_ardrone/pose");
...
void EstimationNode::Loop()
{
	  ...
	  while (nh_.ok())
	  {
		  ...
		  //---Jim---
		  tf::Quaternion q;
		  q.setRPY(s.roll*PI/180.0*2, s.pitch*PI/180.0*2, s.yaw*PI/180.0); 
		  geometry_msgs::Quaternion odom_quat;
  		  tf::quaternionTFToMsg(q, odom_quat); 

		  geometry_msgs::PoseStamped pose_Msg;
		  pose_Msg.header.stamp = s.header.stamp;
		  pose_Msg.header.frame_id = "map";
		  pose_Msg.pose.position.x = s.x;
		  pose_Msg.pose.position.y = s.y;
		  pose_Msg.pose.position.z = s.z;
		  pose_Msg.pose.orientation = odom_quat;
